...
namespace SkyrimScripting::Bind {
    ...
    RE::TESForm* LookupFormID(RE::FormID formID, std::string ScriptName)
    {
	...
	if (form) {
	    ...
	    logger::info(
		"BIND ERROR [{}:{}] ({}) Form ID '{:x}' does not exist", FilePath, LineNumber, ScriptName, formID
	    );
	}
	return {};
    }

    RE::TESForm* LookupEditorID(const std::string& editorID, std::string ScriptName)
    {
	...
	if (form) {
	    ...
	    logger::info(
		"BIND ERROR [{}:{}] ({}) Form Editor ID '{}' does not exist (Are you using po3 Tweaks?)",
		FilePath,
		LineNumber,
		ScriptName,
		editorID
	...
	auto					  isgut = vm->CreateObject(ScriptName, object);
	...
	logger::info(
	    ...
	    ScriptName,
	    isgut,
	    vm->GetObjectBindPolicy()->attachedScripts.size()
	);
    }
	...
	auto niPointer =
	    LocationForPlacingObjects->PlaceObjectAtMe(skyrim_cast<RE::TESBoundObject*, RE::TESForm>(baseForm), true); // TODO back to false
	if (niPointer) {
    ...
    void Bind_GeneratedObject_BaseEditorID(const std::string& baseEditorId, std::string ScriptName)
    {
	auto* form = LookupEditorID(baseEditorId, ScriptName);
	if (form) {
	    Bind_GeneratedObject(ScriptName, form);
	}
    }

    void Bind_GeneratedObject_BaseFormID(RE::FormID baseFormID, std::string ScriptName)
    {
	auto* form = LookupFormID(baseFormID, ScriptName);
	if (form) {
	    Bind_GeneratedObject(ScriptName, form);
	}
    }
    ...
    void Bind_FormID(RE::FormID formID, std::string ScriptName)
    {
	auto* form = LookupFormID(formID, ScriptName);
	if (form) {
	    Bind_Form(form, ScriptName);
	}
    }

    void Bind_EditorID(const std::string& editorID, std::string ScriptName)
    {
	auto* form = LookupEditorID(editorID, ScriptName);
	if (form) {
	    Bind_Form(form, ScriptName);
	}
    }
	...
	if (parentTypeName == "quest" || parentTypeName == "actor" || parentTypeName == "objectreference") {
	    return true;
	}
	return false;
    }
	...
	if (parentName.empty()) {
	    logger::info(
		"BIND ERROR [{}:{}] ({}) Cannot auto-bind to a script which does not `extends` anything",
		FilePath,
		LineNumber,
		ScriptName
	    );
	...
	if (parentName == "quest") {
	    ...
	    logger::info(
		"BIND ERROR [{}:{}] ({}) No default BIND behavior available for script which `extends` {}",
		FilePath,
		LineNumber,
		ScriptName,
		parentName
    ...
    DocstringJsonFilePath.string()); return false;
	    }
	} else {
	    Json::Reader reader;
    ...
    std::filesystem::create_directory(DocstringJsonFilePath.parent_path()); if (InitJson()) { auto& scriptBindComments =
    DocstringJsonRoot["scripts"]; auto& mtimes = DocstringJsonRoot["mtimes"]; auto scriptsFolder =
    std::filesystem::current_path() / "Data\\Scripts";
